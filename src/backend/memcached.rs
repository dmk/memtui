...
// Integration tests (require a running Memcached instance)
#[cfg(all(test, feature = "integration-tests"))]
mod integration_tests {
    use super::*;

    async fn setup_test_backend() -> MemcachedBackend {
        let config = ConnectionConfig {
            id: "test".to_string(),
            name: "Test Memcached".to_string(),
            backend_type: BackendType::Memcached,
            host: std::env::var("MEMCACHED_HOST").unwrap_or_else(|_| "localhost".to_string()),
        ...
        let mut backend = MemcachedBackend::new(config);
        backend
            .connect()
            .await
    ...
    async fn test_info() {
        ...
        // memory_used is u64, so it's always >= 0, just check it exists
        let _memory = info.memory_used;
    }
    ...
    async fn test_set_and_get() {
        ...
        // Cleanup
        let _ = backend.delete("test:key1").await;
    }
    ...
    async fn test_set_with_ttl() {
        ...
        let value = backend.get("test:ttl").await.unwrap();
        assert_eq!(value.data, b"value");

        // Cleanup
        let _ = backend.delete("test:ttl").await;
    }
    ...
    async fn test_get_json_detection() {
        ...
        // Cleanup
        let _ = backend.delete("test:json").await;
    }
    ...
    async fn test_get_many() {
        ...
        // Check that all expected key-value pairs are present (order is not guaranteed)
        let mut found_values = std::collections::HashMap::new();
        for (key, value) in results {
            found_values.insert(key, value.data);
        }
        assert_eq!(found_values.get("test:multi:1").unwrap(), b"value1");
        assert_eq!(found_values.get("test:multi:2").unwrap(), b"value2");
        assert_eq!(found_values.get("test:multi:3").unwrap(), b"value3");
    ...
    async fn test_key_info() {
        ...
        // Cleanup
        let _ = backend.delete("test:info").await;
    }

    #[tokio::test]
    async fn test_execute_raw_not_supported() {
        let backend = setup_test_backend().await;
        let result = backend.execute_raw("stats").await;
