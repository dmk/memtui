...
pub enum EventType {
    ...
    Tick,
    /// Global events delivered to all components (Escape, Quit, etc.)
    Global,
}
    ...
    pub fn is_global(&self) -> bool {
        match self {
            EventKind::Key(key) => {
                ...
                // Escape and quit-related keys are global
                matches!(key.code, KeyCode::Esc)
                    || (key.modifiers.contains(KeyModifiers::CONTROL)
                        && matches!(key.code, KeyCode::Char('c') | KeyCode::Char('q')))
            }
            EventKind::Resize(_, _) => true, // Resize is global
            _ => false,
        }
    }
}
