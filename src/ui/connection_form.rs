...
pub fn render_connection_form(f: &mut Frame, form: &ConnectionForm, error: Option<&str>) {
    ...
    // Title
    let title = Paragraph::new("Add New Redis Connection")
        .style(
            Style::default()
                .fg(Color::Cyan)
    ...
    f.render_widget(host_field, chunks[2]);
    f.render_widget(port_field, chunks[3]);
    f.render_widget(password_field, chunks[4]);
    f.render_widget(database_field, chunks[5]);
